...
namespace TheLargestNumber
{
    public partial class Form1 : Form
    {
        ...
        int[] buffer = { 6,4,0,7,7,1,7,6,5,3,1,3,3,0,6,2,4,9,1,9,2,2,5,1,1,9,6,7,4,4,2,6,5,7,4,7,4,2,3,5,5,3,4,9,1,9,4,9,3,4,9,6,9,8,3,5,2,0,3,
        ...
        2,9,0,9,6,3,2,9,5,2,2,7,4,4,3,0,4,3,5,5,7,6,6,8,9,6,6,4,8,9,5,0,4,4,5,2,4,4,5,2,3,1,6,1,7,3,1,3,5,6,4,0,3,0,9,8,7,1,1,1,
        ...
        9,4,7,6,5,4,5,6,8,2,3,4,8,9,1,2,8,8,3,1,4,2,6,0,7,6,9,0,0,4,2,2,4,2,1,9,0,2,2,6,7,1,0,5,5,6,2,6,3,2,1,1,1,1,1,0,9,3,7,0,
        5,4,4,2,1,7,5,0,6,9,4,1,6,5,8,9,6,0,4,0,8,0,7,1,9,8,4,0,3,8,5,0,9,6,2,4,5,5,4,4,4,3,6,2,4,8,1,2,3,0,9,8,7,8,7,9,9,2,7,2,
        ...
        private void RandomCreateArray(int lenght)
        {
            ...
            txtExampleArray.Text = arrayString;
        }

        private void button1_Click(object sender, EventArgs e)
        {
           
            arrayNew = buffer.ToList();
            int total = 1;
            for (int i = 0; i < arrayNew.Count- Int32.Parse(txtCalculated.Text); i++)
            {
                if(i == 365)
                {
                ...
                total *= arrayNew[i];
                for (int y = 1; y < Int32.Parse(txtCalculated.Text); y++)
                {
                    total *= arrayNew[i + y];
                }
        ...
        private void FindLargestNumber()
        {
            int thehigest = 0;
            int startedOrder = 0;
            for (int i = 0; i < arrayTotal.Count-1; i++)
            {
                if (arrayTotal[i] > thehigest) { thehigest = arrayTotal[i]; startedOrder = i; }
                
            }

            txtResult.Text = thehigest.ToString();
            txtStartedEndedOrder.Text = (startedOrder + 1).ToString() + "/" + (startedOrder + Int32.Parse(txtCalculated.Text)).ToString();
        }

        private void SetArray()
        {
            foreach (var item in txtNewArray.Text.Split(',').ToList())
            {
                if (item != "") arrayNew.Add(Int32.Parse(item));
            }
        }

        private void button2_Click(object sender, EventArgs e)
        {
            RandomCreateArray(Int32.Parse(txtArrayLenght.Text));
        }
    }
}
// SONUÇLAR:
// 1. çıkarım: Ardışık sayıların çarpımlarını bulup, bunları karşılaştırıp en büyüğünü bulmak için hepsini çarpıp bir dizide tutmalıyız ki karşılaştıralım.
// 2. çıkarım: dizi kullanmalıyız ve dizide ki sayıları ard arda çarpmak için de bir döngü kullanmalıyız.
// 3. çıkarım: başka bir döngü daha kullanmalıyız ki; biri döngüyü takip ederken, diğer döngü de ardışık sayıları bulup çarpsın.
// proje, kullanıcının girdiği boyuta,diziye ve hesaplanmasını istediği sayı adedine göre çalışmaktadır. Umarım Beğenirsiniz :)))